...
export DOCKER_IMAGE_NAME ?= tabular-data-viewer
export DOCKER_IMAGE_TAG ?= 0.1.0

OSPARC_DIR:=$(CURDIR)/.osparc

APP_NAME := tabular-data-viewer
...
replay: .cookiecutterrc ## re-applies cookiecutter
	# Replaying git+ssh://git@github.com/ITISFoundation/cookiecutter-osparc-jupyterlab-service ...
	@cookiecutter --no-input --overwrite-if-exists \
		--config-file=$< \
		--output-dir="$(abspath $(CURDIR)/..)" \
		"git+ssh://git@github.com/ITISFoundation/cookiecutter-osparc-jupyterlab-service"
